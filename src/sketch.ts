...
class Bit
{
	...
	constructor(x: number, y: number, type: BitType)
	{
		this.x = x;
		this.y = y;
		this.value = false;
...
class Gate
{
	...
	constructor(x: number, y: number, struct: GateStruct)
	{
		this.x = x;
		this.y = y;
		this.struct = struct;
	...
	calculate(): boolean[]
	{
		...
		this.inputs.forEach(
			i =>
			{
				if(i.connect[0]) i.value = i.connect[0].value;
				else i.value = false;
...
class GateStruct
{
	...
	constructor(inputs: number, outputs: number, name: string, func: (input: boolean[]) => boolean[])
	{
		this.inputs = inputs;
...
function hsvToRgb(hue: number, saturation: number, value: number): [number, number, number]
{
	...
	let rgbP: [number, number, number] | [] = [];
	
	if(hue < 60) rgbP = [ _C, _X, 0 ];
	else if(hue < 120) rgbP = [ _X, _C, 0 ];
	else if(hue < 180) rgbP = [ 0, _C, _X ];
	else if(hue < 240) rgbP = [ 0, _X, _C ];
	else if(hue < 300) rgbP = [ _X, 0, _C ];
	else rgbP = [ _C, 0, _X ];
...
let dragging: Bit | Gate | null = null;
let draggingPos = { x: 0, y: 0 };

let entryBits: Bit[] = [];

let and = new GateStruct(2, 1, "AND",
	(input) =>
	{
...
let gates: Gate[] = [ new Gate(200, 200, and) ];
			...
			else //Check for gate bits
			{
				for(let i = 0; i < gates.length; i++)
				{
					...
					for(let j = 0; j < _bits.length; j++)
					{
						...
						if(_b === dragging) continue;
						if(!_b.overlap(mouseX, mouseY)) continue;
			...
			if(dragging instanceof Bit)
			{
				for(let i = 0; i < gates.length; i++)
				{
					...
					for(let j = 0; j < _bits.length; j++)
					{
						...
						if(_b === dragging) continue; //If the bit is the same as the one we're dragging
						if(!_b.overlap(mouseX, mouseY)) continue;
						
						if(_b.type === dragging.type) continue; //Can not connect an input with an input or an output with an output
